...
import { config, isAdmin, premiumPriceForDays } from "./config.js";
...
export const createBot = (token: string): BotRuntime => {
  ...
  bot.action(/^len:\d+$/, async (ctx) => {
    ...
    if (length === 5 && !store.isPremium(user)) {
      await showScreen(
        store,
        ctx,
        "<b>5 символов доступны только с Premium.</b>\n\nМожно выбрать 6-7 символов или подключить Premium.",
        premiumKeyboard()
      );
      return;
    }
  ...
  bot.action("do_search", async (ctx) => {
    ...
    if (selection.length === 5 && !store.isPremium(fresh)) {
      await showScreen(store, ctx, "<b>5 символов доступны только с Premium.</b>", premiumKeyboard());
      return;
    }
    ...
    if (!store.isPremium(fresh) && fresh.attempts <= 0) {
      await showScreen(
        store,
        ctx,
        "<b>Запросы закончились.</b>\n\nМожно пригласить друга или подключить Premium.",
        Markup.inlineKeyboard([
    ...
    if (store.isPremium(fresh)) {
      ...
      await showScreen(store, ctx, "<b>Запросы закончились.</b>", premiumKeyboard());
      return;
    }
  ...
  bot.action("premium_menu", async (ctx) => {
    ...
    await showScreen(store, ctx, premiumText(user), premiumKeyboard());
  });
  ...
  bot.action(/^buy:\d+$/, async (ctx) => {
    ...
    const days = Number.parseInt(callbackData(ctx).split(":")[1] ?? "31", 10);
    const amount = premiumPriceForDays(days);
    await answer(ctx, "Счёт отправлен");
    await (ctx.telegram as any).callApi("sendInvoice", {
      chat_id: ctx.chat.id,
      title: `${config.premiumName} · ${days} дн.`,
      description: "Premium-доступ к поиску username.",
      payload: `premium:${days}:${amount}`,
      provider_token: "",
      currency: "XTR",
      prices: [{ label: config.premiumName, amount }]
    });
  });
  ...
  bot.on(message("successful_payment"), async (ctx) => {
    ...
    const fresh = store.getUser(ctx.from.id) ?? user;
    await showScreen(store, ctx, `<b>Premium активирован.</b>\n\n${premiumText(fresh)}`, mainKeyboard(isAdmin(ctx.from.id)));
  });
