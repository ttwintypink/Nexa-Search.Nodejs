...
> = {
  turbo: {
    title: "⚡ Турбо",
    short: "быстрая проверка Fragment + Telegram + t.me",
    batch: 8,
    parallelChecks: 3,
    maxTries: 800,
    beautyTries: 6,
  ...
  balance: {
    title: "✅ Баланс",
    short: "стабильный проход: Fragment → Telegram → t.me",
    batch: 8,
    parallelChecks: 3,
    maxTries: 1400,
    beautyTries: 16,
    timeLimitMs: 22_000,
    networkTimeoutMs: 2_200,
    useFragment: true
  },
  strict: {
    title: "🛡 Строгий",
    short: "меньше параллельности, больше точности",
    batch: 6,
    parallelChecks: 2,
    maxTries: 2200,
    beautyTries: 32,
  ...
  beauty: {
    title: "💎 Красивый",
    short: "красивые варианты + стабильные проверки",
    batch: 6,
    parallelChecks: 2,
    maxTries: 1600,
    beautyTries: 80,
    ...
    for (let offset = 0; offset < batch.length; offset += parallel) {
      ...
      for (const item of settled) {
        if (item.status === "fulfilled") {
          ...
          lastReason = errorMessage(item.reason) || "check_failed";
        }
      }
    }
...
const runPublicAvailabilityChecks = async (
  ...
  const telegramCheck = await checkTelegramBotApi(telegram, username, timeoutMs);
...
const checkTelegramBotApi = async (
  ...
  try {
    await withTimeout(telegram.getChat(`@${username}`), timeoutMs, "telegram_timeout");
    ...
    if (message.includes("telegram_timeout")) {
      return verdict(username, false, "telegram_timeout", "telegram");
    }
    if (
...
const checkFragment = async (
  ...
  try {
    ...
    if (!isFragmentUsernameListingUrl(response.finalUrl, username)) {
      ...
      return verdict(username, false, "fragment_not_username_page", "fragment");
    }
    ...
    if (!hasUnderstandableState) {
      return verdict(username, false, "fragment_ambiguous_state", "fragment");
    }

    return verdict(username, true, "fragment_appears_free", "fragment");
  } catch (error) {
    return verdict(username, false, errorMessage(error).includes("abort") ? "fragment_timeout" : "fragment_network_error", "fragment");
  }
};
...
const checkTme = async (username: string, timeoutMs: number): Promise<CandidateCheck> => {
  try {
    ...
    return verdict(username, false, errorMessage(error).includes("abort") ? "tme_timeout" : "tme_network_error", "tme");
  }
};
